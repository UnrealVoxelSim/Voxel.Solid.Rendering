    ...
    const UnrealVoxelSim::Voxel::Api::Region region) noexcept
{
    ...
    return std::array<std::int32_t, 3>{static_cast<std::int32_t>(dimensions[0]),
...
[[nodiscard]] SurfaceId Sample(const Snapshot &snapshot, const std::array<std::int32_t, 3> &local) noexcept
{
    ...
    const Position position{static_cast<std::int32_t>(global[0]), static_cast<std::int32_t>(global[1]),
    ...
    const auto index = (z * static_cast<std::size_t>(dimensions[1]) + y) * static_cast<std::size_t>(dimensions[0]) + x;
    return snapshot.Cells[index];
}

[[nodiscard]] Vertex MakeVertex(const std::array<std::int32_t, 3> &position, const std::size_t axis,
                                const std::int8_t direction, const SurfaceId surface) noexcept
{
...
[[nodiscard]] bool AppendQuad(Mesh &mesh, const std::array<std::int32_t, 3> &origin, const std::size_t axis,
                              const std::size_t u, const std::size_t v, const std::int32_t width,
                              const std::int32_t height, const MaskCell cell)
{
        ...
        for (std::int32_t plane = 0; plane <= dimensions[axis]; ++plane)
        {
            ...
            for (std::int32_t j = 0; j < dimensions[v]; ++j)
            {
                for (std::int32_t i = 0; i < dimensions[u];)
                {
                    ...
                    {
                        bool matches = true;
                        const auto row = static_cast<std::size_t>(j + height) * maskWidth + static_cast<std::size_t>(i);
